...
class VideoAnalysisApp:
    ...
    # Console tee into Progress tab
    self._log_q = queue.Queue(maxsize=1000)
    self.root.after(100, self._drain_log_queue)
    self._orig_stdout, self._orig_stderr = sys.stdout, sys.stderr
    sys.stdout = self._Tee(self._log_q, self._orig_stdout)
    sys.stderr = self._Tee(self._log_q, self._orig_stderr, prefix="[ERR] ")
...
# Results console
try:
    from tkinter import scrolledtext as _scrolled_for_results
    self.results_text = _scrolled_for_results.ScrolledText(self.results_frame, height=20, wrap="word", font=("Consolas", 10))
except Exception:
...
# Progress console
                ...
                try:
                    ...
                    if results is not None:
                        ...
                        if getattr(self, "enable_video_export", tk.BooleanVar(value=True)).get():
                            out_video = os.path.join(os.path.dirname(video_path), f"{video_stem}_ANNOTATED_VIDEO.mp4")
                        ...
                        if getattr(self, "enable_elan_export", tk.BooleanVar(value=True)).get():
                            elan_path = os.path.join(os.path.dirname(video_path), f"{video_stem}_ELAN_TIMELINE.eaf")
                        ...
                        if getattr(self, "enable_csv_export", tk.BooleanVar(value=True)).get():
                            csv_path = os.path.join(os.path.dirname(video_path), f"{video_stem}_FRAME_BY_FRAME.csv")
                            # Use whichever method you actually have:
    ...
    # Populate Results tab with summary + clickable links
    self.root.after(0, lambda: self._show_results_summary(
        success_msg, out_root,
        video_path=out_video if video_saved else None,
        elan_path=elan_path if elan_created else None,
        csv_path=csv_path if csv_saved else None
    ))
self.root.after(0, lambda: messagebox.showinfo("'Looking At' Event Detection Complete", success_msg))
    ...
    def _drain_preview_queue(self):
        ...
        self.root.after(50, self._drain_preview_queue)
    # -------- Progress console tee + Results summary helpers --------
    class _Tee:
        def __init__(self, q, stream=None, prefix=""):
            self.q = q
            self.stream = stream
            self.prefix = prefix
        def write(self, data):
            ...
            text = data.replace("\r", "\n")
            try:
                self.q.put_nowait(self.prefix + text)
            except Exception:
                pass
        def flush(self):
            try:
    ...
    def _append_progress_text(self, text):
    ...
    def _drain_log_queue(self):
        if hasattr(self, "_log_q"):
            chunks = []
            while True:
                try:
                    chunks.append(self._log_q.get_nowait())
                except queue.Empty:
                    break
            if chunks:
                self._append_progress_text("".join(chunks))
        self.root.after(100, self._drain_log_queue)
    ...
    def _open_path(self, path):
        try:
            if os.name == "nt":
                ...
                import subprocess as _subp
                _subp.Popen(["xdg-open", path])
        except Exception as e:
            messagebox.showwarning("Open file", f"Could not open:\\n{path}\\n\\n{e}")

    def _show_results_summary(self, text, out_root, video_path=None, elan_path=None, csv_path=None):
        # Create the results text if missing
        if not hasattr(self, "results_text") or self.results_text is None:
            from tkinter import scrolledtext as _scrolled_for_results
            self.results_text = _scrolled_for_results.ScrolledText(self.results_frame, height=20, wrap="word", font=("Consolas", 10))
            self.results_text.pack(fill="both", expand=True, padx=10, pady=10)
        ...
        try:
            ...
            self.results_text.insert("end", text.strip() + "\\n\\n")
            self.results_text.insert("end", f"Output folder: {out_root}\\n")

            # Insert clickable file links if present
            def _add_link(label, path_str, tagname):
                if not path_str:
                    return
                display = f"{label}: {os.path.basename(path_str)}\\n"
                start = self.results_text.index("end-1c")
                self.results_text.insert("end", display)
                end = self.results_text.index("end-1c")
                self.results_text.tag_add(tagname, start, end)
                self.results_text.tag_config(tagname, foreground="cyan", underline=True)
                self.results_text.tag_bind(tagname, "<Button-1>", lambda e, p=path_str: self._open_path(p))

            _add_link("🎞️ Annotated video", video_path, "link_video")
            _add_link("📄 ELAN timeline",   elan_path,  "link_elan")
            _add_link("📊 CSV file",        csv_path,   "link_csv")

            self.results_text.see("end")
        finally:
            self.results_text.configure(state="disabled")

        # Bring Results tab to front
        try:
